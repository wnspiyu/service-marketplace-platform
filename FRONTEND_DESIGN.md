...
Leaflet with OpenStreetMap tiles requires no API key and has no usage-based billing. The Leaflet API is also well-suited to the two map use cases here: picking a location during registration/task creation and displaying a task alongside nearby providers.
...
Renders an email and password form. On success, redirects to the correct dashboard based on `user.userType`. On failure, displays the error message from the API response.
